...
void rezolvare(Obiect obiecte[], int N, int Gmax)
{
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        {
            greutate = greutate + obiecte[i].greutate;
            printf("Adaugam in ghiozdan obiectul cu greutate %d si profitul %d. \n", obiecte[i].greutate, obiecte[i].profit);
        }
    }
}
...
void rezolvare2(Obiect obiecte[], int N, int Gmax)
{
    ...
    for (int i = 0; i < N; i++)
    {
        ...
        {
            int aux = Gmax - greutate;

            printf("Adaugam in ghiozdan %0.0f%% din obiectul de greutate %d si profit %d. \n", ((float)aux / obiecte[i].greutate) * 100, obiecte[i].greutate, obiecte[i].profit);
        }
    }
}